...
| Command | Description |
| --- | --- |
| `find / -type f \( -name *_hist -o -name *_history \) -exec ls -l {} \; 2>/dev/null` | Find all accessible history files |
...
| `find / -type f -name ".*" -exec ls -l {} \; 2>/dev/null` | Find all hidden files |
...
| `find / ! -path "*/proc/*" -iname "*config*" -type f 2>/dev/null` | Search config files |
| `find / -type f \( -name *.conf -o -name *.config \) -exec ls -l {} \; 2>/dev/null` | Search config files |
| `find / -type f -name "*.sh" 2>/dev/null \| grep -v "src\|snap\|share"` | Find `.sh` scripts |
| `grep -r "word" /starting-path` | Resursively inspect file contents to find instances of "word": |
| `ls -l /tmp /var/tmp /dev/shm` | Find temporary files |
